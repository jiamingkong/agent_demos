        ...
        types.Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "blocks": {
                        "type": "string",
                        "description": "Optional Slack blocks JSON",
                    },
                },
...
async def handle_call_tool(
    ...
    if name == "send_webhook":
        ...
        try:
            ...
            return [
                types.TextContent(
                    type="text", text=f"Success: {response.status_code} {response.text}"
                )
            ]
        ...
        if blocks:
            try:
                ...
                return [
                    types.TextContent(type="text", text="Error: Invalid blocks JSON")
                ]
        try:
            ...
            if data.get("ok"):
                return [
                    types.TextContent(
                        type="text", text=f"Message sent: {data.get('ts')}"
                    )
                ]
            else:
                return [
                    types.TextContent(
                        type="text", text=f"Slack error: {data.get('error')}"
                    )
                ]
        except requests.exceptions.RequestException as e:
            return [types.TextContent(type="text", text=f"Error: {e}")]
        ...
        try:
            ...
            if data.get("ok"):
                return [
                    types.TextContent(
                        type="text",
                        text=f"Message sent: message_id {data['result']['message_id']}",
                    )
                ]
            else:
                return [
                    types.TextContent(
                        type="text", text=f"Telegram error: {data.get('description')}"
                    )
                ]
        except requests.exceptions.RequestException as e:
            return [types.TextContent(type="text", text=f"Error: {e}")]
        ...
        try:
            ...
            return [
                types.TextContent(
                    type="text", text=f"Discord webhook sent: {response.status_code}"
                )
            ]
        except requests.exceptions.RequestException as e:
            return [types.TextContent(type="text", text=f"Error: {e}")]
