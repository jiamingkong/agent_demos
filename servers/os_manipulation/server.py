...
def list_directory(path: str) -> str:
    ...
    try:
        ...
        if not p.exists(): return f"Error: Path '{path}' not found."
        if not p.is_dir(): return f"Error: Path '{path}' is not a directory."
        items = []
...
def move_files_by_regex(source_dir: str, destination: str, pattern: str) -> str:
    ...
    try:
        ...
        if not src_path.is_dir(): return f"Error: Source '{source_dir}' is not a directory."
        if not dst_path.exists(): dst_path.mkdir(parents=True, exist_ok=True)
        if not dst_path.is_dir(): return f"Error: Destination '{destination}' is not a directory."
